...
import numpy as np
import os
from model.load_model import get_model2
from keras.preprocessing import image
from keras.applications.resnet50 import preprocess_input
import json
from datetime import datetime

class CovidDetectorWindow(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('Covid diagnosis')
        ...
        self.imageLabel.setAlignment(Qt.AlignCenter)  # Ensure the image is centered
    ...
    def openFileDialog(self):
        ...
        if fileName:
            ...
            self.imageNameLabel.setText(os.path.basename(fileName)) 
    ...
    def diagnose(self):
        if self.imagePath and self.nameEdit.text().strip():
            ...
            diagnosis_messages = {
                0: 'COVID-19 detected, immediate action required',
                1: 'No COVID or pneumonia detected',
                2: 'No COVID detected but pneumonia suspected, please test for pneumonia.'
            }

            diagnosis = diagnosis_messages[predicted_class]
            self.resultLabel.setText(f'Patient: {self.nameEdit.text()} \nDiagnosis: {diagnosis}')
            if predicted_class != 2:  # Do not save if regular pneumonia detected
